...
/* some modifications by Disastry to compile dll with MSVC6:
 * * removed includes <config.h> "util.h" "memory.h"
 * * moved define DIM from "util.h"
 * * removed LL suffix from 64bit constants
 * * defined byte u32 u64
 * * Modifications by Imad R. Faiad to port to the PGP SDK:-
 * * Deleted GnuPG specific code
 * * Added casts to get rid of MSVC 6 compiler warning errors
 * * Added the necessary code to integrate into the PGP SDK
*/
...
static u64 sbox1[256] = {
    0x02aab17cf7e90c5e /*    0 */,	0xac424b03e243a8ec /*    1 */, /* LL suffix removed - MSVC does not understand it */
...
{
    hd->a = 0x0123456789abcdef;
    hd->b = 0xfedcba9876543210;
    hd->c = 0xf096a5b4c3b2e187;
    hd->nblocks = 0;
    hd->count = 0;
}
...
PGPByte const TIGER192DERprefix[] =
	{ 0x30, 0x28, 0x30, 0x0c, 0x04, 0x08, 0x54, 0x49, 0x47,
	  0x45, 0x52, 0x31, 0x39, 0x32, 0x05, 0x00, 0x04, 0x18 };

static void tiger192Init(void *priv)
{
	tiger_init((TIGER_CONTEXT *)priv);
}
static void tiger192Update(void *priv, void const *bufIn, PGPSize len)
{
    tiger_write((TIGER_CONTEXT *)priv, (byte *)bufIn, len);
}
static void const *tiger192Final(void *priv)
{
    TIGER_CONTEXT *hd = (TIGER_CONTEXT *)priv;
	tiger_final(hd);
    return hd->buf;
}

PGPHashVTBL const HashTIGER192 = {
	"TIGER192", kPGPHashAlgorithm_TIGER192,
	TIGER192DERprefix, sizeof(TIGER192DERprefix),
	24,
	sizeof(TIGER_CONTEXT),
	sizeof(struct{char _a; TIGER_CONTEXT _b;}) -
		sizeof(TIGER_CONTEXT),
	tiger192Init, tiger192Update, tiger192Final
};
